...
import { Engine, Scene, ArcRotateCamera, Vector3, HemisphericLight, SceneLoader, MeshBuilder, Color4, WebXRSessionManager, WebXRState } from '@babylonjs/core';
...
const PRESET_MODELS = [
  { name: "Color Interpolation", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure10_05ColorInterpolation.wrl" },
  { name: "Extruded Room", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure15_08ExtrudedRoom.wrl" },
  { name: "Elevation Grid Mountain", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure16_07ElevationGridMountain.wrl" },
  { name: "Directional Light", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure20_14aDirectionalLight.wrl" },
  { name: "Point Light", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure20_14bPointLight.wrl" },
  { name: "Spot Light", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure20_14cSpotLight.wrl" },
  { name: "Dungeon Room", url: "https://www.web3d.org/x3d/content/examples/Basic/Vrml97Specification/Figure25_12DungeonRoom.wrl" }
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Default box
    MeshBuilder.CreateBox("defaultBox", { size: 2 }, newScene);
      ...
      try {
        ...
        if (isSupported) {
          const xr = await newScene.createDefaultXRExperienceAsync({
            floorMeshes: [newScene.getMeshByName("ground") || MeshBuilder.CreateGround("ground", { width: 100, height: 100 }, newScene)]
          });
          setXrSupported(true);
  ...
  const loadBabylonModel = (fileToLoad: File, ext: string) => {
  ...
  const loadModel = (file: File) => {
    ...
    // Clear existing meshes except camera/light
    scene.meshes.forEach(mesh => {
      if (mesh.name !== "camera" && mesh.name !== "light") {
        mesh.dispose();
      }
    ...
    if (extension === 'wrl') {
      ...
      loadBabylonModel(file, extension);
    }
  };

  const loadPresetModel = async (url: string) => {
    if (!scene) return;
    setIsLoading(true);
    setError(null);
    setIsDefaultModel(false);
    ...
    try {
      ...
      // Clear existing meshes except camera/light
      scene.meshes.forEach(mesh => {
        if (mesh.name !== "camera" && mesh.name !== "light" && mesh.name !== "ground") {
          mesh.dispose();
        }
      });
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 w-full p-4 z-10 flex justify-between items-start pointer-events-none">
        <div className="flex items-center gap-3 bg-black/60 backdrop-blur-md px-4 py-3 rounded-xl border-2 border-dashed border-fuchsia-500 pointer-events-auto shadow-[0_0_15px_rgba(255,0,255,0.5)]">
        ...
        <div className="flex flex-col items-end gap-2 pointer-events-auto">
          ...
          {isDefaultModel && (
            <div className="w-48 bg-black/80 backdrop-blur-md rounded-none border-2 border-dashed border-cyan-400 shadow-[0_0_15px_rgba(0,255,255,0.5)] overflow-hidden flex flex-col mt-2">
              <div className="bg-fuchsia-600/30 px-3 py-2 text-xs font-bold text-yellow-300 uppercase tracking-widest border-b-2 border-dashed border-cyan-400 text-center">
                Preset VRML97 Models
              </div>
              <div className="flex flex-col max-h-64 overflow-y-auto">
                {PRESET_MODELS.map((preset, idx) => (
                  <button
                    key={idx}
                    onClick={() => loadPresetModel(preset.url)}
                    className="text-left px-3 py-2 text-sm text-cyan-300 hover:bg-fuchsia-600/50 hover:text-yellow-300 transition-colors border-b border-cyan-400/30 last:border-0 truncate"
                    title={preset.name}
                  >
                    {preset.name}
                  </button>
